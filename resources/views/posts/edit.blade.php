...
@section('content')
    <h1 class="mt-2 mb-3">Редактировать запись</h1>
    <form method="post" action="{{ route('post.update', ['id' => $post->post_id]) }}"
        ...
        <div class="form-group">
            <textarea class="form-control" name="excerpt"
                      placeholder="Анонс запись" required>{{ $post->excerpt }}</textarea>
        </div>
        <div class="form-group">
            <textarea class="form-control" name="body"
                      placeholder="Текст запись" rows="7" required>{{ $post->body }}</textarea>
        </div>
        <div class="form-group">
